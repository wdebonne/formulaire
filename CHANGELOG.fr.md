...
### Ajouts
- **Accusé de réception au répondant et notification à l'équipe** — toute la mécanique d'envoi existait, mais elle exigeait un modèle Word : il fallait importer un `.docx` pour envoyer un simple e-mail de confirmation. La pièce jointe est désormais **facultative**, et les deux attentes les plus courantes se règlent dans la modale *E-mail d'envoi* sans qu'aucun document n'existe.
  - **Accusé de réception** — un circuit dont le destinataire est un bloc *E-mail* du formulaire écrit au répondant lui-même, à l'adresse qu'il vient de saisir. **Notification à l'équipe** — un circuit avec une adresse fixe et aucune condition part à chaque nouvelle réponse. Les deux peuvent coexister avec les circuits qui, eux, joignent le document.
  - **Rien n'est généré pour rien** — le document n'est produit que si un circuit réellement déclenché le réclame : un formulaire sans modèle ne lit aucun fichier, ne rend aucun `.docx` et ne paie aucun aller-retour de conversion PDF.
  - **Les jetons se résolvent sans modèle** — `{form_title}`, `{entry_date}` et les jetons de questions fonctionnent dans l'objet et le corps que le formulaire ait un modèle ou non : le catalogue des champs est fusionné avec les correspondances enregistrées au lieu de s'y substituer. Les jetons déjà attribués gardent leur bloc — un `.docx` déjà rédigé continue de fonctionner après un renommage — et les questions ajoutées depuis se résolvent tout de même.
  - **Un circuit qui annonce une pièce jointe ne part pas sans elle** — si le modèle est absent ou illisible, ce circuit échoue avec un motif visible plutôt que d'envoyer un « veuillez trouver ci-joint » vide ; les circuits sans pièce jointe de la même réponse partent normalement, un modèle cassé ne doit pas faire taire l'accusé de réception.
- **Anti-spam sur la soumission publique** — l'adresse d'un formulaire publié est publique par nature, et rien n'empêchait un script d'appeler la route d'envoi en boucle : chaque réponse acceptée déclenche un e-mail par circuit d'envoi, un appel par webhook actif, consomme le quota de réponses et laisse une donnée personnelle à purger au titre du RGPD. Trois mesures cumulées y répondent, sans captcha ni service externe, réglables dans la modale **Options** (section *Anti-spam*) et actives par défaut sur tous les formulaires, existants compris.
  - **Champ leurre** — un champ masqué en CSS est ajouté au formulaire public, hors du parcours clavier et ignoré des lecteurs d'écran : aucun répondant ne le rencontre, un robot qui remplit tous les champs le remplit. La réponse est alors jetée et l'appelant reçoit un succès — lui signaler l'échec reviendrait à lui indiquer quel champ laisser vide au prochain essai.
  - **Délai minimum de remplissage** (3 s par défaut) — l'instant d'affichage de la page est **signé par le serveur** et renvoyé à l'envoi : il ne peut donc pas être antidaté, et un script qui appelle directement l'adresse d'envoi sans avoir chargé la page est refusé faute de jeton. Le jeton n'expire volontairement pas : un onglet laissé ouvert toute la nuit doit pouvoir être envoyé le lendemain sans perdre les réponses saisies.
  - **Limitation de débit** (20 réponses par tranche de 10 min et par adresse IP, par formulaire) — même dispositif en mémoire que la vérification du mot de passe d'accès, et pour la même raison : un répondant trop insistant ne doit surtout pas être traité par l'anti-bruteforce des comptes, qui blackliste l'IP sur toute l'application. Le seau est consulté **avant** la porte d'accès, pour qu'un flot soit arrêté avant les requêtes de comptage et surtout avant les webhooks et les e-mails.
- **Les blocs Téléchargement et Signature existent enfin pour le répondant** — tous deux étaient proposés à l'ajout, apparaissaient dans l'aperçu du concepteur et figuraient dans la documentation, mais **le formulaire public n'en rendait aucun champ** : la question s'affichait sans rien à remplir, et rendue obligatoire elle interdisait d'aller au bout du formulaire, puisque la validation exigeait une réponse qu'aucun champ ne permettait de donner. Dans un groupe, le répondant lisait « Type de bloc non supporté dans un groupe ». Les deux blocs sont désormais rendus partout où les autres le sont — question isolée, groupe, bloc répétable — bouton **OK** compris.
  - **Téléchargement** — dépôt par glisser-déposer ou par sélection, envoi immédiat avec indicateur de progression, retrait avant envoi. Le réglage du bloc fixe la **taille maximale** et les **formats acceptés** ; ce choix ne peut que restreindre la liste blanche du serveur, qui exclut de toute façon exécutables et SVG et plafonne à 25 Mo quoi qu'il arrive. Un fichier déposé sur un formulaire fermé, périmé ou protégé par mot de passe est refusé : la route de dépôt refait exactement les contrôles d'accès de la soumission, sans quoi une requête forgée les contournerait.
  - **Les fichiers ne vont pas dans `public/`** — `/api/uploads/[filename]` sert ce dossier **sans aucune authentification**, ce qui conviendrait mal à un justificatif d'identité. Ils vivent dans `storage/response-files/{formulaire}/` (variable `RESPONSE_UPLOAD_DIR`), et la seule porte d'entrée en lecture est une route authentifiée réservée aux personnes ayant accès au formulaire. La réponse ne transporte qu'une référence — nom d'origine, taille, nom stocké —, jamais le document lui-même. **Supprimer une réponse supprime ses pièces jointes** : suppression unitaire, « Tout supprimer », effacement RGPD ciblé, purge par durée de conservation et suppression définitive d'un formulaire. Sans cela, une demande d'effacement aurait laissé le fichier sur le disque après la disparition de la ligne en base.
  - **Signature** — tracé à la souris, au stylet ou au doigt, enregistré en PNG dans la réponse elle-même : quelques kilo-octets, et rien à nettoyer si le formulaire est abandonné. Hauteur de la zone et couleur du tracé réglables. Revenir en arrière sur la question réaffiche la signature déjà donnée plutôt qu'une zone vierge.
  - **En aval, chaque consommateur sait quoi en faire** : le tableau des réponses et l'export CSV affichent le nom du fichier et sa taille ; le détail de la réponse offre le lien de téléchargement et affiche la signature ; les jetons d'un modèle Word, les webhooks, l'export RGPD et les rapports PDF reçoivent un libellé lisible. `resolveDataLabels()` recopie en revanche ces valeurs **intactes** : réduire une pièce jointe à son nom lui ferait perdre sa référence de stockage, donc le fichier.

- **Conversion en PDF par le serveur bureautique de votre NextCloud** — la sortie PDF des modèles Word ne suppose plus d'administrer un conteneur de plus : **Admin → Documents** propose désormais un choix de moteur entre un conteneur Gotenberg et le **NextCloud déjà configuré**, dont le serveur bureautique (Euro-Office, ONLYOFFICE, Nextcloud Office) est installé, authentifié et sauvegardé avec le reste. Les identifiants sont ceux d'*Admin → NextCloud* : il n'y a rien à saisir de plus.
  - **Trois chemins essayés dans l'ordre** — connecteur `eurooffice`, connecteur `onlyoffice`, puis l'API de conversion de NextCloud (qui sert Nextcloud Office). Euro-Office étant un fork d'ONLYOFFICE Docs dont le connecteur NextCloud est une **application distincte**, à l'identifiant près, les deux sont tentés plutôt que de parier sur la variante installée : un connecteur absent ne coûte qu'un 404. Le test dit lequel a répondu. L'API générique vient en dernier parce qu'aucun connecteur bureautique n'y déclare de fournisseur : elle répond, puis échoue faute de fournisseur — sauf si Nextcloud Office est installé à côté.
  - **Le document ne séjourne pas** — il est déposé dans un sous-dossier `.formbuilder-conversion` (le point le range hors de vue dans l'écran des fichiers), converti, rapatrié, puis retiré dans tous les cas, y compris après échec, sans quoi les témoins s'accumuleraient dans un dossier que personne ne pense à ouvrir.
  - **Un refus arrive en JSON sous un HTTP 200** : chaque chemin vérifie les octets `%PDF-` plutôt que de croire le statut ou le `Content-Type`, faute de quoi un message d'erreur partirait sous le nom d'un document.
- **Test de conversion, en plus du test de connexion** — le bouton **Tester la conversion** convertit un vrai document témoin de bout en bout, annonce le chemin qui a répondu et **rend le PDF produit en téléchargement** : une conversion peut « réussir » et sortir une page blanche. Interroger la liste des applications installées ne dirait rien de ce qui se passera au moment de produire un document.
  - Les deux tests ne prouvent pas la même chose, et le verrouillage suit : Gotenberg n'ayant qu'un endpoint de conversion, son `/health` reste concluant. Un NextCloud, lui, peut parfaitement répondre sans qu'aucun serveur bureautique ne lui soit branché — pour ce moteur, **seule une conversion aboutie ouvre l'option PDF**. Une vérification obtenue sur un moteur ne se transporte jamais sur l'autre.
- **Repli sur le `.docx` quand la conversion échoue** — un serveur bureautique en panne faisait jusqu'ici échouer toute la génération : plus rien ne partait. Le document rempli est désormais envoyé **au format `.docx` à la place du PDF**, et la raison de l'échec est conservée sur la réponse (mention ambre sur le détail, reprise dans l'infobulle). Le destinataire préfère un `.docx` à rien, et le répondant a déjà validé son envoi.
- **Catalogue de matériel externe** — un bloc *Choix multiple* ou *Menu déroulant* peut tirer ses options d'une application de gestion du matériel au lieu d'une liste saisie à la main :
  - **La liste suit la date répondue dans le formulaire** — les options n'affichent que ce qu'il reste de disponible sur la période demandée, engagements déjà pris déduits ; changer la date change la liste et les plafonds de quantité. Une liste saisie à la main vieillit sans prévenir : un matériel vendu, une tente déchirée, dix tables achetées, et le formulaire propose encore l'ancien parc.
  - **Périmètre par service, nature et catégorie** — choisis dans des listes alimentées par l'application de gestion plutôt que tapés au clavier, où la faute de frappe ne se voit qu'au premier formulaire vide. Le tri est demandé à l'application elle-même : c'est là qu'est écrit, une fois pour toutes, le rattachement d'un article à un service, et le refaire ici obligerait à recopier cette règle.
  - **Le bloc Quantité suit sa source** — chaque quantité peut être plafonnée au disponible remonté par le catalogue.
  - **Écran d'administration dédié** (`/admin/catalog`) — adresse et jeton d'API, **bouton de test** qui interroge les trois points d'entrée dont dépendent les formulaires (un jeton peut ouvrir la liste des services et se voir refuser le stock : un test s'arrêtant au premier appel annoncerait une réussite démentie au premier formulaire), et **aperçu de ce que répond le catalogue**, filtrable par service, nature, catégorie et période. Les articles à zéro y restent visibles, alors qu'un bloc les masque — c'est précisément ce qui explique pourquoi un article manque dans le formulaire.
  - **Le jeton ne quitte jamais le serveur** — ni le navigateur du répondant, ni l'écran d'administration ne le relisent ; celui-ci sait seulement qu'un jeton existe, et un champ laissé vide conserve celui enregistré. Le formulaire public ne parle pas non plus directement à l'application de gestion : un relais interne s'en charge, borné à un bloc précis d'un formulaire précis pour ne pas devenir un relais ouvert vers elle.
  - Les variables `CATALOG_API_URL` / `CATALOG_API_TOKEN` restent lues **en secours** pour les installations configurées avant cet écran ; ce qui est enregistré dans l'administration prend le dessus, et l'écran signale le cas échéant qu'il est court-circuité.
- **Signature HMAC des webhooks** — un secret partagé renseigné sur un webhook fait accompagner chaque envoi de l'en-tête `X-Webhook-Signature` : un condensat HMAC-SHA256 du corps exactement tel qu'il est transmis, que le destinataire recalcule avec le même secret (convention de GitHub et de Stripe). Sans lui, un destinataire qui ne connaît que l'URL de son webhook ne peut pas distinguer une soumission réelle d'une requête forgée par quiconque a vu cette URL passer. Laissé vide, le webhook part non signé : rien ne change pour les webhooks existants. La soumission, le test depuis l'éditeur et le renvoi manuel signent tous les trois — un test qui réussirait là où la soumission serait refusée serait le pire des comptes rendus, et une reprise après incident doit franchir les mêmes contrôles. L'export JSON d'un formulaire **retire le secret**, un export circulant par courriel et finissant dans un dépôt.
- **Bloc Adresse — autocomplétion restreinte aux communes** — un réglage « Type de saisie » propose désormais *Adresse complète* (comportement d'origine, inchangé par défaut) ou *Ville uniquement*. Le second s'appuie sur le paramètre `type=municipality` de l'API Adresse (BAN) déjà utilisée par le bloc : aucune source de données supplémentaire, aucun bloc distinct à maintenir, et les mêmes garanties de couverture. Chaque suggestion affiche alors le département et la région en seconde ligne (« Lyon » / « 69, Rhône, Auvergne-Rhône-Alpes »), les communes homonymes étant nombreuses ; seul le nom de la ville est enregistré dans la réponse. Au passage, le bloc Adresse ne produisait **aucun champ dans l'aperçu central du builder** — la question s'affichait sans zone de saisie — ce qui est corrigé.
- **Options d'accès d'un formulaire** — une entrée **Options** dans le menu de chaque carte du tableau de bord ouvre une modale qui décide *quand* et *pour qui* le formulaire public accepte des réponses :
  - **Période de disponibilité** — une date de mise en ligne et une date de clôture (heure du serveur, l'une comme l'autre facultatives). Avant la première, le visiteur voit un écran « pas encore ouvert » annonçant la date d'ouverture ; après la seconde, un écran « clôturé ». Les deux messages sont personnalisables.
  - **Mot de passe d'accès** — le formulaire n'est rendu qu'une fois le mot de passe saisi, l'accès étant ensuite mémorisé 12 h sur l'appareil. Le mot de passe est stocké sous forme de condensat bcrypt et **ne quitte jamais le serveur** — la modale sait seulement qu'un mot de passe existe. Le cookie de déverrouillage dérive de ce condensat : **changer le mot de passe révoque donc immédiatement tous les accès déjà accordés**, sans avoir à tenir une liste de sessions. Les essais sont limités à 10 par IP et par formulaire sur 10 minutes, volontairement à l'écart du dispositif anti-bruteforce des comptes pour qu'une faute de frappe d'un répondant ne le bloque jamais sur toute l'application.
  - **Quota de réponses** — ferme le formulaire une fois un nombre maximum de réponses atteint, le compteur étant affiché en direct dans la modale.
  - **Une seule réponse par appareil** — repose sur un cookie déposé après l'envoi ; présenté dans l'interface comme dissuasif et non strict, avec un renvoi vers la restriction par compte pour une vraie garantie.
  - **Réservé aux utilisateurs connectés** — le formulaire n'est servi qu'à un compte authentifié ; le bouton de connexion ramène ensuite au formulaire (la page de connexion honore désormais un paramètre `?redirect=`, restreint aux chemins internes).
  - **Retrait des moteurs de recherche** — une balise `noindex` ; un formulaire protégé par mot de passe ou réservé aux connectés est désindexé de toute façon.
  - Chaque règle est appliquée **côté serveur, au rendu de la page comme à l'envoi d'une réponse** : une requête forgée n'en contourne aucune. Un formulaire protégé ne livre pas ses blocs dans le HTML de l'écran de garde — les questions ne sont transmises qu'une fois l'accès accordé.
  - La carte du tableau de bord signale par un badge « Programmé » ou « Clôturé » qu'un formulaire publié n'accepte en réalité pas de réponses, et par un badge ardoise le nombre d'options actives.
- **Rapports périodiques en PDF, envoyés par e-mail** — un bouton « Rapports » sur la barre de la page des réponses, à côté de « Exporter CSV », ouvre une modale à trois onglets qui produit une synthèse lisible des réponses et l'envoie automatiquement :
  ...
  - **Mise en page** — trois densités (*Compact*, *Normal*, *Aéré*) qui font varier la hauteur des lignes, les cartes d'indicateurs, l'histogramme et les blancs entre les blocs, plus une option pour **commencer chaque section sur une nouvelle page**. Seuls les blancs changent, jamais le corps du texte : tous les décalages du rendu sont posés à la main autour de tailles de police fixes, les faire varier déplacerait les alignements sans rien gagner. *Normal* reproduit exactement la mise en page d'origine — un rapport existant ne bouge donc que si l'on choisit une autre densité.
  - **Réponses libres intégrales** — une option « Reprendre toutes les réponses libres » liste chaque verbatim au lieu d'un échantillon : doublons compris (deux répondants ayant écrit la même chose sont deux réponses), de la plus récente à la plus ancienne, plafonnée à 1 000 par question, le PDF indiquant alors combien de valeurs sont affichées sur le total. Les verbatims sont désormais **repliés sur plusieurs lignes** au lieu d'être tronqués à la première — un verbatim coupé l'était exactement là où on allait le lire.
...
### Modifications
- **Notes présentées comme des notes, et non plus comme un tableau de chiffres** — la section « Valeurs numériques » alignait sept colonnes de valeurs nues (Nb, Min, Moyenne, Médiane, Max, Total), illisibles pour une note de 1 à 5 et dont le cumul n'avait aucun sens. Chaque question numérique a désormais sa propre fiche : la **moyenne rapportée à l'échelle de la question** (« 3,9 / 5 ») positionnée sur une jauge, puis la **répartition note par note** en barres et pourcentages, de la meilleure à la moins bonne, et enfin une ligne discrète pour le nombre de réponses, le minimum, le maximum et la médiane. Les notes que personne n'a choisies restent affichées — « aucun 1 » est une information. La répartition n'apparaît que lorsqu'elle a un sens (valeurs entières, échelle d'au plus onze crans) ; une question de montant libre garde sa moyenne et affiche le cumul, lequel est au contraire masqué sur une note, où additionner des satisfactions ne veut rien dire. Les bornes retenues sont celles **déclarées sur la question**, jamais celles simplement observées dans les réponses : sans cette distinction, une question de budget annoncerait « 1 250 / 1 800 » au gré du plus gros montant reçu.
- **Pourcentages au format français** dans tout le PDF (`66,7 %` et non `66.7 %`), aux côtés des moyennes qui utilisaient déjà la virgule décimale.

### Corrections
- **Déclencheurs de webhook qui n'ont jamais existé** — le sélecteur « Déclencher » proposait *Soumission partielle* et *À l'enregistrement* à côté de *À la soumission*. Aucun chemin d'envoi ne les honorait : choisir l'une de ces deux valeurs rendait le webhook **silencieusement inerte**, actif à l'écran et muet en pratique. Le choix disparaît, la soumission étant le seul déclencheur réel ; un webhook enregistré avec une de ces valeurs **part désormais à la soumission** et l'éditeur l'annonce en clair sur la fiche concernée, plutôt que de corriger en douce. Vérifiez vos webhooks existants si vous vous étiez servi de ce réglage pour suspendre un envoi — c'est l'interrupteur *Actif* qui remplit cet office.
- **Réponses « partielles » annoncées et inexistantes** — le README promettait un « suivi des réponses complètes et partielles » et `Response.status` admettait `partial`, valeur qu'aucun chemin du produit n'a jamais écrite : toute réponse enregistrée est complète. La promesse est retirée de la documentation et du type ; le suivi des réponses inachevées reste à faire, il n'est plus annoncé comme fait.
- **Valeurs structurées déversées en texte brut** — corollaire du point précédent, plusieurs consommateurs dépliaient un objet inconnu clé par clé. Une signature (data-URL base64 de plusieurs kilo-octets) serait ainsi partie entière dans une cellule de l'export RGPD, dans un extrait de recherche RGPD, dans un jeton de document Word et dans les tableaux des rapports PDF. Tous passent désormais par un rendu lisible commun. La recherche RGPD, qui interroge le JSON brut, écarte en outre les correspondances tombées **uniquement dans une signature** — une suite base64 est assez longue pour que n'importe quel nom court finisse par y apparaître ; une correspondance sur une vraie valeur, même que l'extrait ne sait pas reproduire, reste évidemment listée. Au passage, un corps de webhook au format `FORM` écrivait `[object Object]` pour toute valeur structurée — y compris un bloc Quantité ou un groupe, bien avant ces deux nouveaux blocs — et écrit maintenant du JSON.

- **Questions tronquées par les barres du rapport** — dans les sections « Taux de remplissage », « Répartition des réponses » et « Réponses libres », le libellé était confiné à une colonne de 190 pt et coupé net (« Les Activités & Animations — Sur une échell… »), rendant deux questions d'un même groupe indistinguables. Un groupe de barres dont un seul libellé déborde bascule désormais en présentation **empilée** : la question entière sur sa ou ses propres lignes, la barre pleine largeur en dessous. Le choix est pris pour le groupe entier et non barre par barre, sinon une même liste mélangerait des barres démarrant au tiers de la page et d'autres en marge gauche. Le retour à la ligne est calculé à la main plutôt que délégué à pdfkit, dont le retour automatique déplace `doc.y` de son côté et se combine mal avec le dessin libre du reste du rapport.
- **Emoji et symboles illisibles dans les rapports PDF** — un titre de question contenant un emoji (« 🏘️ Informations générales ») ressortait en `Ø<Ø` dans le PDF. Les polices standard du PDF utilisées par pdfkit (Helvetica) sont encodées en **WinAnsi**, limité au Latin-1 étendu : face à un caractère absent de cette table, pdfkit n'échoue pas, il écrit l'octet de poids faible de chaque unité UTF-16 — un emoji, codé sur une paire de substitution, produit donc deux caractères parasites. Aucune police standard ne contient d'emoji et pdfkit ne sait pas changer de police au caractère près à l'intérieur d'un même appel de texte : ces caractères sont désormais **retirés** proprement (`pdfSafeText()`, nouveau `src/lib/pdf-text.ts`), y compris les sélecteurs de variante et les espaces invisibles qu'ils traînent, l'espace résiduel étant résorbé. Les symboles qui ont un équivalent lisible sont transposés plutôt que supprimés (`≥` → `>=`, `→` → `->`, `✔` → `[x]`, `●` → `•`, espaces fines → espace normale), et les accents sont préservés par une précomposition NFC préalable. La correction porte sur **tout texte dynamique** du rapport — titres de questions, options, verbatims, en-têtes de tableau, nom du site, pied de page — ainsi que sur le récapitulatif PDF du panneau RGPD et sur le **nom du fichier** envoyé en pièce jointe, qui ressortait auparavant en séquences d'échappement (`%F0%9F%8F%98`). Les autres exports (Excel, Word, HTML) sont en UTF-8 et n'ont jamais été concernés. Contrepartie assumée : un libellé composé *uniquement* d'emoji ressort vide dans le PDF.
