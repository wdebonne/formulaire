...
- **Automated tests and continuous integration** — roughly 50,000 lines, no test, no workflow: every change rested on a re-read and a manual try. Yet the pure/server module split followed throughout the project already made the business logic directly testable, with no database, no server and no browser. A [Vitest](https://vitest.dev) suite now covers the six modules concerned, and a GitHub Actions workflow checks every push.
  ...
  - **CI checks types, runs the tests and builds the application.** The build is not redundant: it alone catches a server-only module imported from a `'use client'` component, a package that fails to bundle, or a route that does not compile — precisely the regressions that used to slip through.
  - **A second job replays the migrations on a seeded database.** A migration that rebuilds a table passes on an empty database, where the `INSERT ... SELECT` copies no rows, and fails on one holding data: that is how `20260609000000_form_userid_nullable` shipped broken and blocked every existing instance with `P3009`. The case is now replayed on every push.
  - Node's version comes from `.nvmrc`, so the runner, the Docker image and the dev machine cannot drift. `npm run lint` is deliberately not run: with no pre-existing ESLint configuration, the command opens Next's interactive setup and hangs indefinitely.

### Fixed
- **A condition never fired on an option whose label contains a comma** — found while writing the test suite. `src/lib/condition-eval.ts` re-split the stored answer on every comma: an option named "Écran, second" became two values that do not exist, and the e-mail route depending on it stayed silent without reporting anything. The statistics module had solved the same problem on its side; the condition evaluator had not. Consecutive fragments are now re-joined as long as they reconstitute an option the block knows, longest match first — the same reasoning as `tokenize()` in `report-stats.ts`, which meets the problem on the same storage.
- **`@types/node` had stayed on version 20 while everything else runs Node 24** — `.nvmrc`, the Docker image and the dev machine all say 24. This is exactly the drift the "Runtime Version Parity" section warns about: lagging types describe APIs that exist at runtime and are missing at compile time, or the reverse. Aligned on 24.
- **`src/lib/document-fields.ts` was treated as a binary file by git** — a NUL byte was written into it literally as a mapping-key separator, instead of the `\u0000` escape. The consequence: no diff shown for that file any more, and `grep` silently skipping it. The key's value is unchanged; the tests verify it.

### Added
- **Public form accessibility — RGAA 4.1 / WCAG 2.1 AA** — the public renderer carried no ARIA attribute at all. The options of a choice question were bare `<button>`s: a screen reader announced a row of buttons without saying which one was selected, nor whether the question took one answer or several, nor that it was required. For a French local authority or public body, RGAA conformance and the accessibility statement are a legal obligation, not an improvement.
  - **Choice controls announce what they are** — single choice, image choice and Yes/No become a `radiogroup` of `radio`s, multiple choice a group of `checkbox`es, each option carrying its `aria-checked`. The group is tied to the question's heading and carries `aria-required` when it is mandatory. Verified in Chrome's accessibility tree: an option now announces as "Option B, radio button, not checked" rather than "button".
  - **Arrows move through the options, Enter and Space check them** — a choice group is now a single tab stop, like a native radio group. Arrows **move focus without checking**: checking advances to the next question, so selecting on the way would scroll the form at every arrow press. This is the variant the *ARIA Authoring Practices* provide for when selection causes a change of context.
  - **Focus follows the displayed question** — the form replaces one question with the next without changing page: focus stayed on a button that no longer existed and the screen reader kept announcing the previous question. It now moves to the new question, except where an input has already taken focus — stealing it would prevent typing. The thank-you screen, the only feedback a respondent gets about their submission, is announced the same way.
  - **Error messages are announced** — `role="alert"` on every validation message, with `aria-describedby` / `aria-invalid` on the field concerned, so the error reaches the respondent without them having to go back and read it.
  - **Every label is tied to its field** — `<label for>` for native controls, `aria-labelledby` pointing at the heading for button-based sets (choice, rating, quantity, signature, file, date range), in plain questions as well as in groups and repeaters. The asterisk marking a required field, invisible to assistive technology, is backed by a textual mention.
  - **The rest of the journey** — the progress bar becomes a numbered `progressbar`, navigation and icon-only buttons get a name, decorative icons and letter badges leave the accessibility tree, the modals (resume prompt, GDPR notice, file preview) become `dialog`s that take focus and close on Escape, and the gate screen's password field gets a label.

### Fixed
- **Zoom was blocked on mobile** — the site's `viewport` carried `maximum-scale=1` and `user-scalable=no`, which plainly prevented enlarging the page: a WCAG 1.4.4 failure (RGAA 10.4), and an immediate obstacle for anyone with low vision. The original reason — iOS zooming in when a field takes focus — is already handled by the `font-size: 16px` set on the public form's fields, so the restriction served nothing.
- **Enter no longer activated anything from the keyboard** — the global listener that advances to the next question called `preventDefault()` on **every** Enter keypress, including when focus was on a button or an option: the browser then never triggered its activation. On a keyboard alone, no option could be checked and no button pressed with Enter. The listener now stands aside when focus is on an element Enter already activates.
- **Calendar days could not be reached from the keyboard** — the *Advanced date* block rendered each day as a `<td onClick>`: no tab stop, no Enter, no presence in the accessibility tree. Each day is now a `<button>` whose accessible name carries the full date ("12 août 2026" rather than "12"), its selected state and the marker for today; the month and year navigation arrows get labels, and the displayed month is announced when it changes.
- **Image option pictures were announced twice** — the alternative text repeated the label the option already carried. The images become decorative, the accessible name staying on the button, including when label display is switched off — where previously the label was no longer exposed to assistive technology at all.

### Added
- **Resuming an interrupted form** — a tab closed by mistake, a network that drops, a battery that dies, and the whole entry was gone: on a long form — repeaters, an equipment declaration spanning several iterations — that is a guaranteed abandonment, and one response fewer. The entry in progress is now **kept in the respondent's browser** as they type, and a **resume** prompt is offered should they come back.
  - **They get their answers and their place back** — the question they had reached is restored along with the rest, including the current iteration of a repeater. Nothing is restored without their say-so: the prompt comes first, otherwise the first keystroke would overwrite the very draft being offered. *Recommencer* clears the draft and starts from a blank page.
  - **A *Brouillon enregistré* marker** shows while they answer, and only when the save actually landed. Announcing a save that did not happen — quota reached, storage refused by private browsing — would be worse than announcing nothing. Unavailable storage never prevents anyone from answering.
  - **Nothing leaves the respondent's device** — no server-side draft, therefore **no half-filled response created in the database**: nothing more to retain, nothing more to purge under GDPR, no migration. The draft is cleared as soon as the response is sent and expires on its own after 7 days. Opening the page and leaving without typing writes nothing.
  - **A signature is not kept, an attachment is** — a signature is an act, not an entry: re-signing after an interruption is the honest behaviour, and a base64 PNG would fill the browser's quota by itself; the resume prompt says so when the form carries one. An already uploaded file, on the other hand, is found again — the response only carries its reference and the file already sits on the server: resuming spares a re-upload, and redeems a file that would have been left orphaned.
  - **On by default for every form**, existing ones included, and switchable off form by form from the builder's *Paramètres* panel. The builder preview writes to a separate draft: a test entry will never be offered to a respondent.
- **On-screen statistics** — reading a question's completion rate or a choice breakdown meant generating a PDF and opening it. A **Statistiques** page (button of the same name on the responses toolbar) now shows those figures directly, for the period of your choice: key indicators and the change against the previous period, volume over time, the breakdown of every choice question, averages and rating distributions, per-question completion rates, recurring values and the latest verbatims.
  - **These are the PDF report's figures, not a second implementation** — the page calls exactly the computation that feeds the PDF, with the same settings. The screen therefore cannot drift from the document sent by e-mail, and fixing the maths fixes both at once.
  - **The period is chosen in place** — everything, last N days, current month, previous month or a date range, without leaving the page or opening a modal. A closing date set in the report settings caps what is displayed too: past that date, the screen and the PDF describe the same frozen corpus.
  - Responses recorded before label resolution are normalised as elsewhere: an option is never counted twice depending on whether it was stored as its slug or as its label.
- **Excel export, and a period filter on exports** — only CSV existed, and it exported everything indiscriminately: there was no way to pull just the past month. An **Exporter** button now opens a modal where the **format** (`.xlsx` workbook or CSV) and the **period** (everything, last N days, current month, previous month, date range) are chosen.
  - **Both formats describe the same table** — one row per response, one column per question, group inner fields and repeater iterations expanded into separate columns, choice labels resolved rather than slugs. The format only changes the encoding, never the content; building the table is now shared by both.
  - **The modal states what will be downloaded** — row and column counts computed with the function the server uses, before the click. The period bounds are recomputed **server-side**: the browser picks a mode, it never designates a range of rows directly.
  - **A repeater iteration is no longer lost when its first field is empty** — the iteration count was inferred from the presence of the first inner field alone; left empty because it was optional, it made the whole iteration vanish from the export. Any iteration where some field carries a value is now exported.
  - **An unanswered cell is empty, not filled with a dash** — in a spreadsheet, a "-" cannot be counted, filtered or summed. The `.xlsx` workbook also sizes its columns to their content, without which a column of verbatims opens too narrow to read.
- **Automatic purge of expired data** — the retention periods (36 months for responses, 365 days for the activity log) only turned into deletions if somebody remembered to click "Purge". Retention that depends on a human gesture is not retention: it is deferred non-compliance. Both purges now run off the **in-process timer** that already sent the periodic reports, with a switch in each admin screen.
  - **One switch per screen** — *Purge automatique quotidienne* in **GDPR** (`/admin/gdpr`) and in **Activity log** (`/admin/logs`), right under the retention period and next to the count of what has expired: you see what enabling it will delete before enabling it. The last automatic run is shown under the switch.
  - **Off by default, upgrades included** — until now, "limit the retention period" only ever meant "count it and let me purge". Turning that setting into automatic deletion would erase, at the first restart after an upgrade, responses nobody consented to lose. Enabling it stays an explicit decision.
  - **One implementation, two triggers** — the button and the timer call the same code: same attachments deleted along with the responses, same cutoff **recomputed server-side** and never taken from the client, same audit entry. A purge is never invisible: the activity screen tells a manual trigger from an automatic one and records how many items went, never what they held.
  - **Once a day, not once a tick** — the cutoff moves by 24 h, not by five minutes. The timestamp is stamped even after a failure, otherwise a momentarily unavailable database would have the same purge retried every five minutes. A purge that deleted nothing writes nothing, or the log would pile up 365 entries a year saying so.
  - **For deployments driven by an external cron** — `POST /api/internal/retention/run` triggers the same pass as the timer (shared secret, like `/api/internal/reports/run`). The switches stay sovereign: that route purges nothing while automatic purging is off.
  - **A single timer for everything** — reports and purges share one scheduler (`SCHEDULER`, `SCHEDULER_INTERVAL_MINUTES`; the historical `REPORT_SCHEDULER*` names are still honoured). A failure on the reports does not skip the purges, and vice versa.
  - **Right-to-erasure deletions are now logged too** (`gdpr.erasure`) — how many responses went and who did it, never the search term nor a response value.
- **Acknowledgement to the respondent and notification to the team** — the whole sending machinery was already there, but it demanded a Word template: you had to import a `.docx` just to send a plain confirmation e-mail. The attachment is now **optional**, and the two most common expectations are set up in the *E-mail d'envoi* modal with no document in sight.
  - **Acknowledgement** — a circuit whose recipient is an *Email* block of the form writes to the respondent themselves, at the address they have just typed in. **Team notification** — a circuit with a fixed address and no condition fires on every new response. Both can sit alongside the circuits that do attach the document.
  - **Nothing is generated for nothing** — the document is only produced when a circuit that actually matched asks for it: a form without a template reads no file, renders no `.docx` and pays for no PDF conversion round-trip.
  - **Tokens resolve without a template** — `{form_title}`, `{entry_date}` and question tokens work in the subject and body whether the form has a template or not: the field catalog is merged with the saved mappings instead of being replaced by them. Tokens already assigned keep their block — a `.docx` already written keeps working after a rename — and questions added since resolve all the same.
  - **A circuit that announces an attachment does not go out without it** — if the template is missing or unreadable, that circuit fails with a visible reason rather than sending an empty "please find attached"; the attachment-free circuits of the same response go out normally, since a broken template must not silence the acknowledgement.
- **Anti-spam on public submission** — a published form's address is public by nature, and nothing stopped a script from calling the submit route in a loop: every accepted response fires one e-mail per delivery route, one call per enabled webhook, eats into the response quota and leaves personal data to purge under GDPR. Three cumulative measures now answer that, with no captcha and no external service, configurable in the **Options** modal (*Anti-spam* section) and on by default for every form, existing ones included.
  - **Honeypot field** — a CSS-hidden field is added to the public form, kept out of the keyboard path and ignored by screen readers: no respondent ever meets it, a bot filling every field fills it. The response is then discarded and the caller gets a success — telling it the request failed would tell it which field to leave empty next time.
  - **Minimum fill time** (3 s by default) — the moment the page was rendered is **signed by the server** and sent back on submission, so it cannot be backdated, and a script POSTing straight to the submit endpoint without loading the page is refused for lack of a token. The token deliberately never expires: a tab left open overnight must still be submittable the next morning without losing the answers typed into it.
  - **Rate limit** (20 responses per 10-minute window per IP address, per form) — same in-memory mechanism as the access-password check, and for the same reason: an over-eager respondent must never be handed to the account anti-bruteforce, which blacklists the IP application-wide. The bucket is consulted **before** the access gate, so a flood is stopped ahead of the counting queries and, above all, ahead of the webhooks and e-mails.
- **The File upload and Signature blocks finally exist for the respondent** — both were offered in the add-block dialog, appeared in the builder preview and were listed in the documentation, yet **the public form rendered no field for either**: the question showed up with nothing to fill in, and once marked required it made the form impossible to finish, since validation demanded an answer no field could provide. Inside a group the respondent read "Block type not supported in a group". Both blocks are now rendered everywhere the others are — standalone question, group, repeater — **OK** button included.
  - **File upload** — drag and drop or file picker, immediate upload with a progress indicator, removal before submitting. The block settings set the **maximum size** and the **accepted formats**; that choice can only narrow the server's allow-list, which excludes executables and SVG regardless and caps everything at 25 MB. A file dropped on a closed, expired or password-protected form is refused: the upload route repeats exactly the access checks the submission performs, without which a forged request would bypass them.
  - **Files do not go into `public/`** — `/api/uploads/[filename]` serves that folder **with no authentication whatsoever**, which would suit a proof of identity poorly. They live in `storage/response-files/{form}/` (`RESPONSE_UPLOAD_DIR`), and the only way in is an authenticated route restricted to people with access to the form. The response carries a reference only — original name, size, stored name — never the document itself. **Deleting a response deletes its attachments**: single deletion, "Delete all", targeted GDPR erasure, retention purge and permanent form deletion. Without that, an erasure request would have left the file on disk after the database row was gone.
  - **Signature** — drawn with a mouse, a stylus or a finger, stored as a PNG inside the response itself: a few kilobytes, and nothing to clean up if the form is abandoned. Pad height and pen colour are configurable. Going back to the question redisplays the signature already given rather than a blank pad.
  - **Downstream, every consumer knows what to do with them**: the response table and the CSV export show the file name and its size; the response detail offers the download link and displays the signature; Word template tokens, webhooks, the GDPR export and the PDF reports all receive a readable label. `resolveDataLabels()`, on the other hand, copies those values **untouched**: reducing an attachment to its name would strip its storage reference, and with it the file.

- **PDF conversion by your NextCloud's office server** — PDF output for Word templates no longer means administering one more container: **Admin → Documents** now offers a choice of engine between a Gotenberg container and the **already-configured NextCloud**, whose office server (Euro-Office, ONLYOFFICE, Nextcloud Office) is installed, authenticated and backed up along with everything else. The credentials are the ones from *Admin → NextCloud*: there is nothing more to enter.
  - **Three paths, tried in order** — the `eurooffice` connector, the `onlyoffice` connector, then NextCloud's own conversion API (which serves Nextcloud Office). Euro-Office being a fork of ONLYOFFICE Docs whose NextCloud connector is a **separate application**, identical routes aside from the app id, both are attempted rather than betting on which variant is installed: a missing connector only costs a 404. The test says which one answered. The generic API comes last because no office connector registers a provider with it: it answers, then fails for lack of a provider — unless Nextcloud Office sits alongside.
  - **The document does not linger** — it is deposited in a `.formbuilder-conversion` subfolder (the leading dot keeps it out of sight in the Files screen), converted, fetched back, then removed in every case, including after a failure, otherwise witnesses would pile up in a folder nobody thinks to open.
  - **A refusal arrives as JSON under HTTP 200**: every path checks for the `%PDF-` magic bytes rather than trusting the status or the `Content-Type`, failing which an error message would go out under the name of a document.
- **A conversion test, on top of the connection test** — the **Test conversion** button converts a real witness document end to end, announces which path answered and **hands back the produced PDF as a download**: a conversion can "succeed" and render a blank page. Querying the list of installed applications would say nothing about what happens when a document is actually produced.
  - The two tests do not prove the same thing, and the gating follows: Gotenberg exposing a single conversion endpoint, its `/health` remains conclusive. A NextCloud, however, can answer perfectly well with no office server attached to it — for that engine, **only a successful conversion opens the PDF option**. A verification obtained on one engine never carries over to the other.
- **Fallback to `.docx` when conversion fails** — a broken office server used to fail the whole generation: nothing went out at all. The filled document is now sent **as a `.docx` in place of the PDF**, and the reason for the failure is kept on the response (amber note on the detail, echoed in the tooltip). The recipient would rather have a `.docx` than nothing, and the respondent has already submitted.
- **External equipment catalog** — a *Multiple choice* or *Dropdown* block can draw its options from an equipment-management application instead of a hand-typed list:
  - **The list follows the date answered in the form** — options only show what is still available over the requested period, commitments already made deducted; changing the date changes the list and the quantity caps. A hand-typed list ages without warning: an item sold, a torn tent, ten tables bought, and the form still offers last year's stock.
  - **Scope by service, kind and category** — picked from lists fed by the management application rather than typed by hand, where a typo only shows up on the first empty form. The filtering is asked of the application itself: that is where an item's attachment to a service is written once and for all, and redoing it here would mean copying that rule.
  - **The Quantity block follows its source** — each quantity can be capped at what the catalog reports as available.
  - **A dedicated admin screen** (`/admin/catalog`) — API address and token, a **test button** that queries all three endpoints the forms depend on (a token can open the service list and still be refused on stock: a test stopping at the first call would announce a success the first form would contradict), and a **preview of what the catalog answers**, filterable by service, kind, category and period. Items at zero stay visible there, whereas a block hides them — which is exactly what explains why an item is missing from the form.
  - **The token never leaves the server** — neither the respondent's browser nor the admin screen reads it back; the latter only ever learns that a token is set, and leaving the field empty keeps the stored one. The public form does not talk to the management application either: an internal relay does, bound to one specific block of one specific form so it cannot become an open relay towards it.
  - `CATALOG_API_URL` / `CATALOG_API_TOKEN` are still read **as a fallback** for installations configured before this screen existed; what is saved in the admin takes precedence, and the screen says so when it is being bypassed.
- **Webhook HMAC signature** — a shared secret set on a webhook makes every delivery carry an `X-Webhook-Signature` header: an HMAC-SHA256 digest of the body exactly as transmitted, which the receiver recomputes with the same secret (the GitHub and Stripe convention). Without it, a receiver who only knows their webhook URL cannot tell a genuine submission from a request forged by anyone who saw that URL go by. Left empty, the webhook is sent unsigned: nothing changes for existing webhooks. Submission, the test from the editor and manual replay all sign — a test succeeding where the submission would be refused would be the worst possible report, and a replay after an incident must pass the same checks. A form's JSON export **strips the secret**, an export travelling by e-mail and ending up in a repository.
- **Address block — autocomplete restricted to towns** — a new "Input type" setting offers *Full address* (the original behaviour, still the default) or *Town only*. The latter relies on the `type=municipality` parameter of the Adresse (BAN) API the block already queries: no extra data source, no separate block to maintain, and the same coverage guarantees. Each suggestion then shows its department and region on a second line ("Lyon" / "69, Rhône, Auvergne-Rhône-Alpes"), since same-named towns are common in France; only the town name is stored in the response. Along the way, the address block rendered **no input at all in the builder's centre preview** — the question appeared with no field beneath it — which is now fixed.
...
- **Periodic PDF reports, delivered by e-mail** — a "Rapports" button on the responses page toolbar, next to "Export CSV", opens a three-tab modal that turns responses into a readable summary and mails it automatically:
  ...
  - **Layout** — three densities (*Compact*, *Normal*, *Airy*) changing row heights, indicator cards, chart height and the white space between blocks, plus an option to **start each section on a new page**. Only the blanks change, never the type size: every offset in the renderer is hand-computed around fixed font sizes, so scaling type would shift alignments for nothing. *Normal* reproduces the original layout exactly, so an existing report only changes when someone picks another density.
  - **Free-text answers, in full** — a "Include every free-text answer" option lists every verbatim rather than a sample: duplicates included (two respondents writing the same thing are two answers), newest first, capped at 1 000 per question with the PDF stating how many are shown out of the total. Verbatims now **wrap over several lines** instead of being ellipsed after one — a truncated verbatim was being cut exactly where it was about to be read.
...
### Fixed
- **Webhook triggers that never existed** — the "Trigger" selector offered *Partial submission* and *On save* next to *On submission*. No sending path honoured them: picking either made the webhook **silently inert**, enabled on screen and mute in practice. The choice is gone, submission being the only real trigger; a webhook stored with one of those values **now fires on submission**, and the editor says so plainly on the webhook concerned rather than quietly rewriting it. Check your existing webhooks if you had used that setting to pause a delivery — the *Enabled* switch is what does that.
- **"Partial" responses announced but non-existent** — the README promised "partial and completed response tracking" and `Response.status` allowed `partial`, a value no code path has ever written: every recorded response is complete. The claim is removed from the documentation and from the type; tracking unfinished responses remains to be built, and is no longer advertised as done.
- **Structured values dumped as raw text** — a corollary of the above: several consumers unfolded an unknown object key by key. A signature (a base64 data URL several kilobytes long) would therefore have gone out whole into a GDPR export cell, a GDPR search snippet, a Word document token and the PDF report tables. All of them now go through one shared readable rendering. The GDPR search, which queries the raw JSON, additionally discards matches falling **inside a signature only** — a base64 string is long enough for any short name to eventually turn up in it; a match on a real value, even one the snippet cannot reproduce, is of course still listed. Along the way, a `FORM`-encoded webhook body wrote `[object Object]` for any structured value — including a Quantity block or a group, long before these two new blocks — and now writes JSON.
