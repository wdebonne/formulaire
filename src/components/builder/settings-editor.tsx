...
export function SettingsEditor() {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      {/* Logo section */}
      <div className="pt-4 border-t">
        <h4 className="font-medium text-sm mb-4">Logo</h4>

        {/* Show logo */}
        <div className="flex items-center justify-between mb-4">
          <div>
            <Label>Afficher le logo</Label>
            <p className="text-xs text-gray-500">Logo de la personnalisation</p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
        ...
        {settings.showLogo && (
          <div className="space-y-2 ml-4 mb-4">
            ...
            <select
              value={settings.logoPosition ?? 'top'}
              onChange={(e) => updateSettings({ logoPosition: e.target.value as 'top' | 'bottom' })}
              className="w-full px-3 py-2 text-sm border rounded-md"
            >
              <option value="top">En haut</option>
              <option value="bottom">En bas</option>
        ...
        {settings.showLogo && (
          <div className="space-y-2 ml-4">
            ...
            <select
              value={settings.logoAlignment ?? 'left'}
              onChange={(e) => updateSettings({ logoAlignment: e.target.value as 'left' | 'center' | 'right' })}
              className="w-full px-3 py-2 text-sm border rounded-md"
            >
              <option value="left">À gauche</option>
